...
def load_art(path: Path) -> list[str]:
    try:
        ...
        raise SystemExit(f"Could not find {path}. Put the bik1 art in text.md.") from error
...
def blink_state(cycle_index: int) -> str:
    if cycle_index in (42, 47):
        return "half"
    if cycle_index in (43, 44, 45, 46):
        return "closed"
    return "open"
...
def crop_to_terminal(rows: list[str]) -> list[str]:
    columns, lines = shutil.get_terminal_size(fallback=(132, 48))
    if columns > 10:
...
def render_frame(
    ...
    *,
    ...
    axis_x = round(math.sin(phase) * 5.0)
    axis_y = round((math.sin(phase * 1.2 - 0.5) + 1.0) * 1.5)
    shoulder_sway = round(math.sin(phase * 2.0 + 0.25) * 1.0)
    base_indent = 2

    figure = blink_art(art, cycle_index)
    rows: list[str] = ["" for _ in range(axis_y)]
    show_sign = sign and cycle_index == 0
    middle = len(figure) // 2

    for y, line in enumerate(figure):
        row_wave = round(math.sin(phase * 1.3 + y * 0.16) * 0.8)
        body_sway = shoulder_sway if y < middle else -shoulder_sway
        offset = base_indent + axis_x + row_wave + body_sway
        row = move_line(line, offset)
        if show_sign and 3 <= y < 3 + len(SIGN_LINES):
            row = append_skyy_sign(row, y - 3)
        rows.append(row)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Animate the bik1 art from text.md.")
    ...
    parser.add_argument("--once", action="store_true", help="print the 64 frames once")
