    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            # Matrix rain effect
            rain_speed = 0.3 + (y % 5) * 0.2
            brightness = math.sin(phase * rain_speed + x * 0.3 + y * 0.1)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_matrix_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_matrix_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Matrix visualization mode.")
