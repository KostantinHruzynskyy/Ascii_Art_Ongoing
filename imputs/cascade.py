...
def render_cascade_frame(
    ...
    *,
    ...
    for y, line in enumerate(art):
        delay = y * 0.08
        wave = math.sin(phase + delay) * 3.0
        offset = max(0, round(wave))
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_cascade_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_cascade_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cascade visualization mode.")
