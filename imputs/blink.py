...
def noise(x: int, y: int, frame: int) -> int:
    return (x * 41 + y * 59 + frame * 31) % 100


def render_blink_frame(
    art: list[str],
    frame_number: int,
    *,
    crop: bool = False,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            blink_speed = 0.5 + (y % 3) * 0.3
            threshold = 50 + 30 * math.sin(phase * blink_speed + x * 0.2)
            n = noise(x, y, frame_number)
            if n < threshold:
                chars.append(char)
            else:
                chars.append(EMPTY)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_blink_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_blink_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Random blinking visualization mode.")
