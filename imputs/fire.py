    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            # Fire effect: flickering based on position and time
            flicker = math.sin(phase * 5 + x * 0.5 + y * 0.3)
            intensity = 0.5 + 0.5 * math.sin(phase * 3 + x * 0.2)
            
            if flicker > 0.3 and intensity > 0.4:
                # Replace with fire characters based on intensity
                if intensity > 0.8:
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_fire_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_fire_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fire visualization mode.")
