...
def render_chromatic_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    shift = int(math.sin(2 * math.pi * cycle_index / FRAME_COUNT) * 3)
    rows: list[str] = []
    for y, line in enumerate(art):
        if y % 3 == 0:
            rows.append(move_line(line, shift))
        elif y % 3 == 1:
            rows.append(line)
        else:
            rows.append(move_line(line, -shift))
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_chromatic_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_chromatic_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Chromatic visualization mode.")
