...
def render_edge_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    rows: list[str] = []
    for y, line in enumerate(art):
        chars = list(line)
        for x in range(len(chars)):
            if chars[x] not in (EMPTY, ' '):
                is_edge = (y == 0 or y == len(art) - 1 or 
                          x == 0 or x == len(line) - 1 or
                          (y > 0 and art[y-1][x] in (EMPTY, ' ')) or
                          (y < len(art)-1 and art[y+1][x] in (EMPTY, ' ')))
                if not is_edge:
                    chars[x] = EMPTY
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_edge_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_edge_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Edge visualization mode.")
