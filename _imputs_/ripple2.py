...
def render_ripple2_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    rows: list[str] = []
    for y, line in enumerate(art):
        chars = list(line)
        offset = int(math.sin(cycle_index / FRAME_COUNT * 2 * math.pi + y * 0.5) * 2)
        chars = chars[offset:] + chars[:offset] if offset > 0 else chars[:abs(offset)] + chars[abs(offset):]
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_ripple2_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_ripple2_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Ripple2 visualization mode.")
