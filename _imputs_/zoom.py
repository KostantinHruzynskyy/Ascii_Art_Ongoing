...
def render_zoom_frame(
    ...
    *,
    ...
    zoom = 1.0 + 0.5 * math.sin(phase)
    rows: list[str] = []

    art_height = len(art)
    art_width = max(len(line) for line in art)

    center_y = art_height / 2
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            rel_y = (y - center_y) / zoom
            rel_x = (x - center_x) / zoom

            new_y = center_y + rel_y
            new_x = center_x + rel_x

            if 0 <= int(new_y) < art_height and 0 <= int(new_x) < art_width:
                source_char = art[int(new_y)][int(new_x)]
                if source_char != EMPTY and source_char != " ":
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_zoom_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_zoom_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Zoom visualization mode.")
