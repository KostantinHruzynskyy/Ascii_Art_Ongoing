...
def render_sepia_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    chars = ' .:;+=*#%@'
    rows: list[str] = []
    for line in art:
        new_line = []
        for c in line:
            if c not in (EMPTY, ' '):
                idx = (hash(c) + frame_number) % len(chars)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_sepia_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_sepia_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sepia visualization mode.")
