...
def render_scale_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    scale = 1 + 0.5 * math.sin(2 * math.pi * cycle_index / FRAME_COUNT)
    rows: list[str] = []
    for line in art:
        new_len = max(1, int(len(line) * scale))
        if scale > 1:
            rows.append((line * (new_len // len(line) + 1))[:new_len])
        else:
            rows.append(line[:new_len])
    if crop:
        rows = crop_to_terminal(rows)
    return '
'.join(rows)
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_scale_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_scale_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scale visualization mode.")
