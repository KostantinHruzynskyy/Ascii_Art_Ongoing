...
def render_terminal_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            if char == EMPTY or char == " ":
                chars.append(char)
                continue
            effect = math.sin(phase + x * 0.1 + y * 0.1)
            if effect > 0.3:
                chars.append(char)
            else:
                chars.append(EMPTY)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_terminal_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_terminal_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Terminal visualization mode.")
