...
def render_emboss_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    rows: list[str] = []
    for y, line in enumerate(art):
        chars = list(line)
        for x in range(len(chars)):
            if chars[x] not in (EMPTY, ' '):
                prev = art[y][x-1] if x > 0 else EMPTY
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_emboss_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_emboss_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Emboss visualization mode.")
