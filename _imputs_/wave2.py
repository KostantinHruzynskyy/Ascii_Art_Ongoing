...
def render_wave2_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    rows: list[str] = []
    for y, line in enumerate(art):
        offset = int(math.sin(cycle_index / FRAME_COUNT * 2 * math.pi + y * 0.3) * 4)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_wave2_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_wave2_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Wave2 visualization mode.")
