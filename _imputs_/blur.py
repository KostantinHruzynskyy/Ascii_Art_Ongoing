...
def render_blur_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    blur_amount = (cycle_index / FRAME_COUNT) * 3
    rows: list[str] = []
    for y, line in enumerate(art):
        shifted = EMPTY * int(blur_amount) + line
        rows.append(shifted[:len(line)] if len(shifted) > len(line) else shifted.ljust(len(line), EMPTY))
    if crop:
        rows = crop_to_terminal(rows)
    return '
'.join(rows)
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_blur_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_blur_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Blur visualization mode.")
