    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            vortex = math.sin(angle * 2 + dist * 0.3 - phase * 2)
            if vortex > 0:
                chars.append(char)
            else:
                chars.append(EMPTY)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_vortex_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_vortex_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Vortex visualization mode.")
