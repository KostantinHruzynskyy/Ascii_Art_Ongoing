...
def render_kaleidoscope_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    angle = cycle_index / FRAME_COUNT * 2 * math.pi
    rows: list[str] = []
    for y, line in enumerate(art):
        chars = list(line)
        for x in range(len(chars)):
            if chars[x] not in (EMPTY, ' '):
                mirror = (x + int(math.sin(angle) * 3)) % 2 == 0
                chars[x] = chars[x] if mirror else EMPTY
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_kaleidoscope_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_kaleidoscope_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Kaleidoscope visualization mode.")
