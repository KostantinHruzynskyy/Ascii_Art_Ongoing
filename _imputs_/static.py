...
def render_static_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    rng = random.Random(frame_number)
    rows: list[str] = []
    for line in art:
        chars = [c if rng.random() > 0.3 else EMPTY for c in line]
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_static_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_static_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Static visualization mode.")
