...
def render_threshold_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    threshold = (frame_number % 5) + 3
    rows: list[str] = []
    for line in art:
        chars = list(line)
        count = sum(1 for c in chars if c not in (EMPTY, ' '))
        if count < threshold:
            chars = [EMPTY] * len(chars)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_threshold_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_threshold_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Threshold visualization mode.")
