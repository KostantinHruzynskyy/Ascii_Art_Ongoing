...
def render_glitch2_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    ...
    for line in art:
        ...
        if rng.random() < 0.4:
            ...
            chars[pos:pos+3] = [EMPTY, EMPTY, EMPTY]
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_glitch2_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_glitch2_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Glitch2 visualization mode.")
