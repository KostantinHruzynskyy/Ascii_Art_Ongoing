#!/usr/bin/env python3
"""
Cyberpunk mode - neon colors, glitch effects, and futuristic aesthetic
"""
...
def render_cyberpunk_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            # Cyberpunk effect with color shifts
            wave = math.sin(phase * 3 + x * 0.15 + y * 0.2)
            glitch = rng.random() < 0.05
            
            if glitch:
                # Glitch effect
                chars.append(random.choice(CYBER_CHARS))
            elif wave > 0.5:
                # Bright neon
                chars.append("█")
            elif wave > 0.2:
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_cyberpunk_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_cyberpunk_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Cyberpunk visualization mode.")
