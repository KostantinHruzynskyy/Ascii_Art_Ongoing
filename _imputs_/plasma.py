    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            # Plasma effect
            v1 = math.sin(x * 0.1 + phase)
            v2 = math.sin(y * 0.1 + phase)
            v3 = math.sin((x + y) * 0.1 + phase)
            v4 = math.sin(math.sqrt(x * x + y * y) * 0.1 + phase)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_plasma_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_plasma_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plasma visualization mode.")
