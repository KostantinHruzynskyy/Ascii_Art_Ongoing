...
def render_noise_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    rng = random.Random(frame_number)
    rows: list[str] = []
    for line in art:
        chars = list(line)
        for i in range(len(chars)):
            if chars[i] not in (EMPTY, ' ') and rng.random() < 0.5:
                chars[i] = EMPTY
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_noise_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_noise_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Noise visualization mode.")
