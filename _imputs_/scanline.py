...
def render_scanline_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    rows: list[str] = []
    for y, line in enumerate(art):
        if (y + cycle_index) % 2 == 0:
            rows.append(line)
        else:
            rows.append(EMPTY * len(line))
    if crop:
        rows = crop_to_terminal(rows)
    return '
'.join(rows)
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_scanline_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_scanline_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Scanline visualization mode.")
