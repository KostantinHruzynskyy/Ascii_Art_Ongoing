...
def render_pixelate_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    block_size = 3
    rows: list[str] = []
    for y, line in enumerate(art):
        new_line = []
        for x in range(0, len(line), block_size):
            block = line[x:x+block_size]
            if any(c not in (EMPTY, ' ') for c in block):
                new_line.append(block[0] if block[0] not in (EMPTY, ' ') else EMPTY)
            else:
                new_line.append(EMPTY * len(block))
        rows.append(''.join(new_line).ljust(len(line), EMPTY))
    if crop:
        rows = crop_to_terminal(rows)
    return '
'.join(rows)
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_pixelate_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_pixelate_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pixelate visualization mode.")
