...
def render_sharpen_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    rows: list[str] = []
    for y, line in enumerate(art):
        ...
        for x in range(len(chars)):
            ...
            for dy in [-1, 0, 1]:
                for dx in [-1, 0, 1]:
                    ny, nx = y + dy, x + dx
                    if 0 <= ny < len(art) and 0 <= nx < len(line):
                        if art[ny][nx] not in (EMPTY, ' '):
                            neighbors += 1
            if neighbors < 3:
                chars[x] = EMPTY
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_sharpen_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_sharpen_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sharpen visualization mode.")
