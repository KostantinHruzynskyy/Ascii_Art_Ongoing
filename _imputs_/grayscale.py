...
def render_grayscale_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    chars = ' .:-=+*#%@'
    rows: list[str] = []
    for line in art:
        new_line = []
        for c in line:
            if c not in (EMPTY, ' '):
                idx = hash((frame_number, c)) % len(chars)
                new_line.append(chars[idx])
            else:
                new_line.append(c)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_grayscale_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_grayscale_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Grayscale visualization mode.")
