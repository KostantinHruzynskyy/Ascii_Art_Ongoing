...
def render_earthquake_frame(
    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        shake = int(math.sin(phase * 10 + y * 0.5) * 3)
        if rng.random() > 0.7:
            shake += rng.randint(-2, 2)
        
        shifted = move_line(line, shake)
        rows.append(shifted)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_earthquake_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_earthquake_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Earthquake visualization mode.")
