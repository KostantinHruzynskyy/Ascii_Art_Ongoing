...
def render_crt_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    rows: list[str] = []
    for y, line in enumerate(art):
        chars = list(line)
        scanline = (y + cycle_index) % 3 == 0
        if scanline:
            chars = [EMPTY if c != EMPTY and c != ' ' else c for c in chars]
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_crt_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_crt_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Crt visualization mode.")
