...
def render_bounce_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    bounce = abs(math.sin(2 * math.pi * cycle_index / FRAME_COUNT)) * 5
    offset = int(bounce)
    rows: list[str] = []
    for line in art:
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_bounce_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_bounce_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bounce visualization mode.")
