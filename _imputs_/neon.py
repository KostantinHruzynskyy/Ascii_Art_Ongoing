    ...
    *,
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            # Neon glow effect
            glow = math.sin(phase * 2 + x * 0.1 + y * 0.1)
            if glow > 0.5:
                chars.append("█")
            elif glow > 0.2:
                chars.append("▓")
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_neon_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_neon_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Neon visualization mode.")
