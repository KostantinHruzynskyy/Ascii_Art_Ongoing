#!/usr/bin/env python3
"""
Hacker mode - combines matrix rain with green terminal aesthetics,
random character flickering, and scanline effects for a classic hacker look.
"""
...
# Hacker-style characters for random flicker
HACKER_CHARS = "!@#$%^&*()_+-=[]{}|;:,.<>?/\\`~" + "01"
    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            if char == EMPTY or char == " ":
                # Random hacker character flicker in empty spaces
                if rng.random() < 0.02:
                    chars.append(random.choice(HACKER_CHARS))
                else:
                    chars.append(char)
                continue

            # Matrix rain effect with green terminal aesthetic
            rain_speed = 0.2 + (y % 7) * 0.15
            brightness = math.sin(phase * rain_speed + x * 0.2 + y * 0.15)

            # Add some random glitch/flicker
            glitch = rng.random() < 0.03

            if brightness > 0.6 and not glitch:
                # Bright green - main character
                chars.append(char)
            elif brightness > 0.3 or glitch:
                # Medium brightness - dimmer block or random char
                if glitch:
                    chars.append(random.choice(HACKER_CHARS))
                else:
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_hacker_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_hacker_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Hacker visualization mode.")
