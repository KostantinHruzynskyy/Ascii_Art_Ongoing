    ...
    for y, line in enumerate(art):
        ...
        for x, char in enumerate(line):
            ...
            dist_from_center = math.sqrt(
                (x - len(line) / 2) ** 2 + (y - len(art) / 2) ** 2
            )
            wave = math.sin(phase - dist_from_center * 0.2)
            if wave > 0:
                chars.append(char)
            else:
                chars.append(EMPTY)
...
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [
        render_pulse_frame(art, frame_number, crop=crop)
...
def animate(art: list[str], delay: float, crop: bool) -> None:
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_pulse_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pulse visualization mode.")
