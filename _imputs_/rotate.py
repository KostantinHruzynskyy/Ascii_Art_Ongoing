...
def render_rotate_frame(art: list[str], frame_number: int, *, crop: bool = False) -> str:
    cycle_index = (frame_number - 1) % FRAME_COUNT
    shift = cycle_index % len(art[0]) if art else 0
    rows: list[str] = []
    for line in art:
        rows.append(line[shift:] + line[:shift])
    if crop:
        rows = crop_to_terminal(rows)
    return '
'.join(rows)
def build_frames(art: list[str], *, crop: bool) -> list[str]:
    return [render_rotate_frame(art, frame_number, crop=crop) for frame_number in range(1, FRAME_COUNT + 1)]
    ...
    try:
        while True:
            for frame_number in range(1, FRAME_COUNT + 1):
                clear_screen()
                frame = render_rotate_frame(art, frame_number, crop=crop)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rotate visualization mode.")
